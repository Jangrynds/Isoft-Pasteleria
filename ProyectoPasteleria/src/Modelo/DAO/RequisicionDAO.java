...
public class RequisicionDAO {
    ...
    public RequisicionDAO(Supplier<Connection> connectionSupplier) {
    ...
    public void llenarSolicitantes(JComboBox combo, String departamento) {
        try {
            Connection con = getConnection();
            String sql = "SELECT e.nombre, e.apellidoP, e.apellidoM "
                    + "FROM empleado e "
    ...
    public void mostrarMaterialesSolicitados(JTable tabla, int idPedido) {
        ...
        String sql = "SELECT i.nombreIngrediente, "
                + "SUM(dp.cantidad * r.cantidad) AS cantidadNecesaria, "
                + "i.medida, i.precioUnitario "
                + "FROM detallePedido dp "
                + "INNER JOIN receta r ON dp.idProducto = r.idProducto "
                + "INNER JOIN ingredientes i ON r.idIngrediente = i.idIngrediente "
                + "WHERE dp.idPedido = ? "
                + "GROUP BY i.idIngrediente, i.nombreIngrediente, i.medida, i.precioUnitario";
        ...
        try {
            ...
            while (rs.next()) {
                double cantidad = rs.getDouble("cantidadNecesaria");
    ...
    public int obtenerIdIngrediente(String nombreIngrediente) {
        try {
            Connection con = getConnection();
            String sql = "SELECT idIngrediente FROM ingredientes WHERE nombreIngrediente = ?";
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public boolean guardarDetalleRequisicion(int idRequisicion, int idIngrediente, double cantidad, double costoUnitario, double total) {
        try {
            Connection con = getConnection();
            String sql = "INSERT INTO detalleRequisicion (idRequisicion, idIngrediente, cantidad, costoUnitario, total) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public int guardarRequisicion(Requisicion req) {
        try {
            Connection con = getConnection();
            String sql = "INSERT INTO requisicion (fecha, idDepartamento, idPedido, solicitante, observaciones, total, entregadoPor, recibidoPor) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

            PreparedStatement ps = con.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);

            ps.setDate(1, new java.sql.Date(req.getFecha().getTime()));
            ps.setInt(2, req.getIdDepartamento());
            ps.setInt(3, req.getIdPedido());
            ps.setString(4, req.getSolicitante());
            ps.setString(5, req.getObservaciones());
            ps.setDouble(6, req.getTotal());
            ps.setString(7, req.getEntregadoPor());
            ps.setString(8, req.getRecibidoPor());
    ...
    public void mostrarRequisiciones(JTable tabla) {
        ...
        String sql = "SELECT r.idRequisicion, r.fecha, d.nombreDepartamento, r.solicitante, r.idPedido, r.total "
                + "FROM requisicion r "
                + "INNER JOIN departamento d ON r.idDepartamento = d.idDepartamento";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                datos[2] = rs.getString("nombreDepartamento");
                datos[3] = rs.getString("solicitante");
                datos[4] = rs.getString("idPedido");
    ...
    public boolean eliminarRequisicion(int idRequisicion) {
        try {
            ...
            String sqlDetalle = "DELETE FROM detalleRequisicion WHERE idRequisicion = ?";
            PreparedStatement psDetalle = con.prepareStatement(sqlDetalle);
            psDetalle.setInt(1, idRequisicion);
            psDetalle.executeUpdate();

            String sqlReq = "DELETE FROM requisicion WHERE idRequisicion = ?";
    ...
    public void mostrarDetalleRequisicion(JTable tabla, int idRequisicion) {
        ...
        String sql = "SELECT i.nombreIngrediente, i.medida, dr.cantidad, dr.costoUnitario, dr.total "
    ...
    public boolean descontarInventario(int idIngrediente, double cantidad) {
        try {
            Connection con = getConnection();
            String sql = "UPDATE ingredientes SET cantidad = cantidad - ? WHERE idIngrediente = ?";
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public boolean verificarExistencia(int idIngrediente, double cantidadNecesaria) {
        try {
            Connection con = getConnection();
            String sql = "SELECT cantidad FROM ingredientes WHERE idIngrediente = ?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, idIngrediente);
            ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                double existencia = rs.getDouble("cantidad");
